"""Sports plugin schema — betting/odds/player tables.

Registered against the domain-general core (tools.schema.core). Every
table here is sports vocabulary; none of it is required by the core
lifecycle. The plugin also owns the ``hypothesis_sports_ext`` side table
that carries the sports-specific columns (sport, market_type,
edge_threshold) which used to be welded onto core ``hypotheses``.

Registered via tools.schema.registry.register_plugin_schema().
"""
...
# ──────────────────────────────────────────────────────────────
# Plugin-owned side table for the core claim lifecycle.
#
# ``hypotheses.sport/market_type/edge_threshold`` were the weld that made a
# Bitcoin or protein-folding claim impossible to store. They now live here,
# keyed to the core row. Core schema never changes when a new domain adds
# its own extension table.
#
# Back-compat: migration 013 copies existing values into this table and
# compat views (see tools/schema/compat.py) keep ``SELECT sport FROM
# hypotheses`` working during the transition.
# ──────────────────────────────────────────────────────────────
...
# Backtests use this to filter data by rule era. Sports-owned: other
# domains register their own era tables if they need one.
# ──────────────────────────────────────────────────────────────
