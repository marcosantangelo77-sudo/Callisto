...
def test_w7_fixed_batch_rerun_does_not_duplicate_rows(tmp_path):
    """FIXED (routing pass 2026-08-23): write_routing_scores dedupes on
    (role, model, task_class, question_id), so a resumed/rerun batch no
    longer doubles n or inflates the basis label."""
    ...
    assert agg["n"] == 1                     # deduped: one observation, one question


def test_w8_fixed_routing_judges_the_task_class_slice(tmp_path):
    """FIXED (routing pass 2026-08-23): decide() takes task_class and judges
    each candidate on its (role, task_class) slice. Here A's SYNTHESIS
    record is poor (0.45, worse than chance) while its rival B has great
    classification measurements — under the old role-wide pooling B's
    classification scores leaked into this synthesis decision. Post-fix the
    synthesis call sees A's real (bad) slice record and B as unmeasured.
    (The discriminating specialist-isolation pins live in
    ...
    # A synthesis call judges A on its own poor slice: B (wide chance-centred
    # exploration draw around 0.25) beats A's measured 0.45 most draws.
    wins = {"A": 0, "B": 0}
    for seed in range(50):
        ...
        d = pol.decide("pipeline", [CandidateModel(name="A", tier="t"),
                       ...
                       task_class="research_synthesis")
        wins[d.model] += 1
    assert wins["B"] > wins["A"]             # B not poisoned IN by classification
