"""Pin: NBA impact+redistribute helpers extracted to tools.injury.impact_nba.
...
model.py keeps dataclasses, player_impact, and redistribute_usage;
those two dispatchers import the extracted helpers.
...
IMPACT = ROOT / "tools" / "injury" / "impact_nba.py"
ANALYSIS = ROOT / "tools" / "injury" / "analysis.py"
NFL_MLB = ROOT / "tools" / "injury" / "impact_nfl_mlb.py"
...
def test_impact_nba_defines_the_four_functions():
    impact_fns = _top_level_func_names(IMPACT)
    for name in MOVED:
        assert name in impact_fns, name
    analysis_fns = _top_level_func_names(ANALYSIS)
    nfl_fns = _top_level_func_names(NFL_MLB)
    for name in MOVED:
        assert name not in analysis_fns, name
        assert name not in nfl_fns, name


def test_model_keeps_dataclasses_and_dispatchers():
...
def test_model_reexports_impact_nba_names():
    reexported = _reexported_names(MODEL, "impact_nba")
    for name in MOVED:
...
def test_reexports_are_the_same_objects():
    ...
    import tools.injury.impact_nba as impact

    for name in MOVED:
        assert getattr(model, name) is getattr(impact, name), name
        assert getattr(model, name).__module__ == "tools.injury.impact_nba"


def test_dispatchers_import_extracted_helpers():
    pi_imported = _func_imports_from(MODEL, "player_impact", "impact_nba")
    assert "_nba_player_impact" in pi_imported
    ru_imported = _func_imports_from(MODEL, "redistribute_usage", "impact_nba")
    assert "_nba_usage_redistribution" in ru_imported
...
def test_compat_shim_and_package_still_resolve():
    ...
    for name in MOVED:
        assert getattr(impact, name).__module__ == "tools.injury.impact_nba"


def test_line_counts():
    model_n = MODEL.read_text(encoding="utf-8").count("\n")
    impact_n = IMPACT.read_text(encoding="utf-8").count("\n")
    assert model_n < 320, model_n
    assert impact_n >= 220, impact_n
