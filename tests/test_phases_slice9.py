"""Pin: phase_system_improvement moved into tools.loop.phases.system_improve.
...
SYSIMP = ROOT / "tools" / "loop" / "phases" / "system_improve.py"
...
def test_system_improve_defines_async_phase():
    assert "phase_system_improvement" in _async_defs(SYSIMP)
    fn = next(
        n
        for n in ast.parse(SYSIMP.read_text(encoding="utf-8")).body
        if isinstance(n, ast.AsyncFunctionDef) and n.name == "phase_system_improvement"
    )
...
def test_post_live_still_exposes_name():
    ...
    assert "phase_system_improvement" in names
    tree = ast.parse(POST.read_text(encoding="utf-8"))
    exposed = False
    for n in tree.body:
        if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == "phase_system_improvement":
            exposed = True
        elif isinstance(n, ast.ImportFrom):
            for a in n.names:
                if (a.asname or a.name) == "phase_system_improvement":
                    exposed = True
    assert exposed


def test_neither_system_improve_nor_post_live_imports_autonomous():
    pkg = ROOT / "tools" / "loop" / "phases"
    for path in (
        SYSIMP, POST, IMPL, pkg / "__init__.py", pkg / "repair.py",
...
def test_line_counts():
    ...
    sysimp_n = SYSIMP.read_text(encoding="utf-8").count("\n")
    assert post_n < 700, post_n
    assert sysimp_n >= 150, sysimp_n
...
def test_live_execute_not_introduced_into_system_improve():
    src = SYSIMP.read_text(encoding="utf-8")
    assert "CALLISTO_ALLOW_LIVE_EXECUTE" not in src
    assert "phase_live_execute" not in _async_defs(SYSIMP)
...
def test_runtime_gate_closed_and_delegate_identity():
    ...
    from tools.loop.phases import system_improve

    class _Loop:
        hypothesis_manager = object()

    assert asyncio.run(phases_impl.phase_live_execute(_Loop())) is None
    assert phases_impl.phase_system_improvement is post_live.phase_system_improvement
    assert phases_impl.phase_system_improvement.__module__ == "tools.loop.phases.post_live"
    assert system_improve.phase_system_improvement.__module__ == "tools.loop.phases.system_improve"
    assert system_improve.phase_system_improvement is not post_live.phase_system_improvement
