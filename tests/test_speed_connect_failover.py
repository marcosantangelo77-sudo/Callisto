"""SPEED run 12 (2026-08-23): connect-phase failures fail over immediately.
...
loopback refusals) and would double a full connect-timeout hang on hosts that
drop SYN instead of refusing. `callisto.py` builds a fresh ProviderRouter per
ask, so the cooldown ladder never got traction and every question re-paid the
tax on gpu1/gpu1_fast-first task classes.
...
Invariants:
  ...
  3. 5xx and 429 semantics are byte-identical to runs 8/10 (covered there;
     the 5xx timing pin is repeated here as a tripwire).
...
def test_5xx_backoff_timing_unchanged_tripwire():
    """Tripwire: the run-12 edit must not disturb the 5xx path."""
    calls = {"n": 0}
