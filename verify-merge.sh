#!/bin/bash
# Post-merge guard. Catches the failure classes that a human reviewer — me —
# has repeatedly missed by eye:
#   1. a merge that DELETED a public function another branch added
#   2. test files that vanished
#   3. a source file that shrank sharply (a stale branch overwriting newer work)
#   4. imports that no longer resolve
# Run after EVERY merge, before pushing. Exit 1 means do not push.
set -u
cd ~/Documents/GitHub/Callisto || exit 1
PREV=${1:-HEAD~1}
fail=0
...
gone=$(comm -23 <(git show "$PREV" --name-only --pretty=format: 2>/dev/null | grep '^tests/.*\.py$' | sort -u) \
                 <(ls tests/*.py 2>/dev/null | sort -u))
missing=""
...
echo "── source files that shrank >25%"
for f in $(git diff --name-only "$PREV"..HEAD -- '*.py' | grep -vE '^tests/'); do
  [ -f "$f" ] || continue
  o=$(git show "$PREV:$f" 2>/dev/null | wc -l | tr -d ' '); n=$(wc -l < "$f" | tr -d ' ')
...
# BEFORE merging and only fails on failures that are NEW. Without this the
# guard reverted 117 commits of good work over a pre-existing missing dep.
...
for p in list(pathlib.Path("tools").rglob("*.py")) + list(pathlib.Path("agp").rglob("*.py")):
    ...
    try: importlib.import_module(mod)
        ...
        # Missing THIRD-PARTY deps are an environment gap, not merge damage.
        # A missing FIRST-PARTY module means a merge deleted a file.
