"""Hypothesis auto-promote body extracted from promote mixin.

``HypothesisPromotionMixin.auto_promote`` stays defined on the mixin as a
thin delegate so ``hasattr`` pins keep passing. The body lives here so
``tools/hypothesis/promote.py`` can keep shrinking without changing
behaviour.

auto_promote is DIAGNOSE-ONLY with respect to edge_threshold /
signal_generated: it may log a threshold diagnosis and hold, but never
writes to edge_threshold or signal_generated.

``check_promotion_readiness`` stays on ``HypothesisSignificanceMixin`` —
do not copy it onto ``HypothesisPromotionMixin``.
``review_live_hypotheses`` stays a thin delegate on this mixin.
