"""Hypothesis promotion-readiness body extracted from significance mixin.

``HypothesisSignificanceMixin.check_promotion_readiness`` stays defined on
the mixin as a thin delegate so ``hasattr`` pins keep passing. The gate
