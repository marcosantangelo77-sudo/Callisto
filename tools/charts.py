...
def _guarded_text(value):
    ...
    if not isinstance(value, str) or not value:
        return value
    if value[0] in _FORMULA_LEADS:
        if _is_number(value):
            # Numeric strings ("-5", "+3") are legitimate data: store them
            # as real numbers so Excel reads them as values, never re-parses
            # them as formulas, and they never surface as "'-5".
...
def build_workbook(spec: dict) -> bytes:
    ...
    for sheet_name, table in spec.get("data", {}).items():
        ...
        cols = table["columns"]
        ws.append([_guarded_text(c) for c in cols])
        for c in ws[1]:
            c.font = bold
    ...
    for r, m in enumerate(spec.get("model", []), start=2):
        ws.cell(row=r, column=1, value=_guarded_text(m.get("label", "")))
        ws.cell(row=r, column=2, value=_guarded_text(m["cell"]))
        # column C and ModelLive are the ONLY formula-write sites: they carry
        # spec["model"][*]["formula"], which is generated by OUR code, never
    ...
    for m in spec.get("model", []):
        if _valid_cell(m["cell"]):
            ...
            if not header.value:
                header.value = _guarded_text(m.get("label", m["cell"]))
    ...
    for sc in spec.get("scenarios", []):
        for k, v in sc["overrides"].items():
            ws.append([_guarded_text(sc["name"]), _guarded_text(k),
                       _guarded_text(v)])
