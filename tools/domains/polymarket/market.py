...
class PolymarketAdapter:
    ...
    def public_search(self, query: str, *, limit_per_type: int = 10) -> dict:
        """Gamma full-text search across events, tags and profiles.

        This is the endpoint the app's own search bar uses; it is how a
        sub-question finds WHICH contracts exist for its topic before any
        book is read. Unauthenticated, GET, no key.

        Returns {'events': [dict], '_fetch': provenance}. Events carry
        ticker/slug/title plus nested markets when Gamma includes them.
        """
        query = str(query).strip()
        if not query:
            raise ValueError("public_search needs a non-empty query")
        url = self.source.build_url("/public-search", {
            "q": query,
            "limit_per_type": max(1, min(int(limit_per_type), 50)),
            "events_status": "active",
        })
        data, rec = self.source.get_json(url)
        events = data.get("events") or [] if isinstance(data, dict) else []
        return {"events": events, "_fetch": _fetch_meta(rec)}
