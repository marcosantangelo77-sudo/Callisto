...
| # | Dimension (10 pts) | Morning | **Master now** | Evidence on `373352e` |
| --- | --- | --- | --- | --- |
...
Haircut notes (why not 80): `/odds/*` dump GETs still loopback-only by bind,
not by auth; `tools/edge.py` still has a third Kelly; kernel `MODEL_LADDER`
≠ ProviderRouter; `ResearchLoop` still lives in one file; `callisto ask`
is unchanged; no overnight soak of the new gates on Marco's box.
...
Structural: `kelly_core` + `kelly_binary` wrapper, dashboard research face,
`tools.loop.PhaseFailureLedger`.
